...
"""LLAMA-3.2-1B Prefill on MLIR-AIE (NPU2)
...
Architecture:
  1. Compile phase: For each unique kernel config, produce mlir-aie text
     (post-stitching aie/aiex dialect) and compile via **aiecc** to an ELF.
  2. Run phase: Load cached .elf via xrt.elf / xrt.ext.kernel, execute on NPU.
...
def compile_all_kernels(cache, config, seq_len, cpu_attn=True):
    """Pre-compile all unique prefill kernels to cache (mlir-aie -> aiecc -> ELF)."""
    ...
    print(f"\n{'='*60}")
    print(f"Compiling unique prefill kernels (seq_len={seq_len})...")
    print(f"{'='*60}\n")

    cache.compile_and_cache(
        "rms_gemms_rope",
        aie_ir_gen.build_rms_gemms_rope_ir(
    ...
    cache.compile_and_cache(
        "o_ffn",
        aie_ir_gen.build_o_ffn_ir(seq_len, emb_dim, hidden_dim,
                                  verbose=cache.verbose),
        instance_name="o_ffn",
    )
    ...
    if not cpu_attn:
        cache.compile_and_cache(
            "flash_attn",
            aie_ir_gen.build_flash_attn_ir(
