...
"""LLAMA-3.2-1B BF16 Inference on MLIR-AIE (NPU2).

Unified script: NPU prefill + NPU decode with NPU LM Head.

This duplicates the MLIR-AIR llama32_1b inference but compiles each kernel
with **aiecc only** (no aircc at run/load time). The IR generation step
is currently a thin wrapper around the existing multi-launch builders +
aircc's AIR passes (see kernel_builder/aie_ir_gen.py) so this project
serves as the low-level seam at which the user can hand-edit the cached
`*.npu.air.mlir` files or replace each builder with hand-written
placed-iron python without touching the rest of the pipeline.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Override the HF repo (or local dir) for weights+tokenizer. "
        "Bypasses the --model -> meta-llama/... mapping. Useful for non-gated "
        "mirrors like unsloth/Llama-3.2-1B-Instruct.",
    )
    parser.add_argument(
        "--quant",
        type=str,
    ...
    if args.hf_model_id and args.quant != "bf16":
        parser.error("--hf-model-id cannot be combined with --quant awq")
